<%-- 
    Document   : Eleccion
    Created on : 30-nov-2014, 21:13:29
    Author     : Alám
--%>
...
<script type="text/javascript">
    ...
    <div id="templatemo_main">
      ...
      <body>
        <%
    int decision = Integer.parseInt(request.getParameter("eleccion"));
    out.println("<H1> INICIA SESION JUEGO</H1><BR><BR>");
    if(decision == 1)
        out.println("<FORM action=ConexionBaseDatos method=POST>");
    else if(decision == 2)
        out.println("<FORM action=jugar method=POST>");
    out.println("Nombre: <Input type='text' name='nom'><BR><BR>");
        out.println("Contraseña: <Input type='text' name='pass'><BR><BR>");
        out.println("<Input type='submit' name='Dados' value='Entrar'>");
      %>
      <input type="button" onclick=" location.href='http://localhost:8080/ProyectoWEB/Mysql.jsp' " value="Regresar" name="boton" /> 
     </body>
        </div>
